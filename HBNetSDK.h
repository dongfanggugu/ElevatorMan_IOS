...
void HB_NET_SetLogLevel(int nLevel);
    
// 等待时间，单位毫秒
BOOL HB_NET_SetConnectTime(int waitTime, int tryTimes);
...
#define HB_ENCODE_TYPE_GB2312       1
#define HB_ENCODE_TYPE_UTF8         2
...
typedef void (*PHB_NET_DATA_PROC)(LONG lCommand, void* pData, void* pContex);
...
typedef struct ST_HB_NET_VODCOND
{
    DWORD       dwSize;
    DWORD       dwChannel;                  // 通道号 [0, n-1],n:通道数
    DWORD       dwType;                     // 录像类型:1-手动，2-定时，4-移动，8-报警，0xFF-全部
    DWORD       dwLoadMode;                  // 回放下载模式 1-按时间， 2-按名字
    union
    {
        struct
        {
            HB_NET_TIME struStartTime;         // 最多一个自然天
            HB_NET_TIME struStopTime;          // 结束时间最多到23:59:59,
            // 即表示从开始时间开始一直播放
            char cReserve[16];
        }byTime;
...
}HB_NET_VODCOND, *LPHB_NET_VODCOND;

long HB_NET_Vod(long lUserID, LPHB_NET_VODCOND lpVodCond);

BOOL HB_NET_StopVod(long lVodHandle);

// dwFrameType: 0,所有帧; 1,仅I帧
BOOL HB_NET_VodGetFrame(long lVodHandle, DWORD dwFrameNum, DWORD dwFrameType);
...
typedef struct ST_HB_NET_DEVICECFG
{
    ...
    BYTE    byVGANum;
    BYTE    byUSBNum;
    DWORD   dwRes;                                  // nvr 12用于区分nvr与其他设备
...
#define HB_NET_GET_ZEROVENCCFGNVR  0x105
#define HB_NET_SET_ZEROVENCCFGNVR  0x106
    
    typedef struct  {
        short   width;
        short   height;
    }HB_NET_VEN_RESOLUTION;
    ...
    typedef struct ST_HB_NET_VENCCFGNVR
    {
        BYTE    byStreamAudio;                           // 码流类型: 0-无音频, 1-有音频
        BYTE    byBitRateType;                          // 码率类型: 0-变码率, 1-定码流
        BYTE    byPicQuality;                           // 图像质量, 1~6, 1最好, 6最差
        BYTE    reserve;
        HB_NET_VEN_RESOLUTION resolution;
        DWORD   dwBitRate;      // 码率 0-100K, 1-128K, 2-256K, 3-512K, 4-1M, 5-1.5M, 6-2M,
        // 7-3M, 8-4M ...
        // 其他:码率值[32~2^32] 以K为单位
        WORD    wFrame;                                 // 帧率 wFrame/wTimeBase,一般为2～30
        WORD    wTimeBase;                              //
        DWORD   dwQuant;                                // 量化因子 1~31
    }HB_NET_VENCCFGNVR, *LPHB_NET_VENCCFGNVR;
    
    typedef struct ST_HB_NET_ZEROVENCCFGNVR
    {
        DWORD   dwSize;
        DWORD   dwEnable;                               // 是否启用复合通道
        BYTE    byChlList[HB_MAX_CHANNUM];              // 选择通道
        HB_NET_VENCCFGNVR struVencCfg;                     // 复合通道视频参数
        BYTE    bySwitchTime;                           // 切换时间
        BYTE    byStreamType;
        BYTE    reserve;
        BYTE    byFormat;                               // 画面格式, 0-off, 1-1画面
        // 4-2*2画面, 9-3*3画面, 16-4*4画面
        HB_NET_VEN_RECT channelRect[128];
    }HB_NET_ZEROVENCCFGNVR, *LPHB_NET_ZEROVENCCFGNVR;
...
typedef	struct
{
    ...
    HB_NET_IVA_POLYGON obj_height;                  // 目标（人）在图像中的高度，默认85
...
int HB_NET_SetChlName(long lUserID, long lChlNO, char *strChlName, int nNameLen);
...
BOOL HB_NET_SetVoiceEncodeType(LONG lUserID, int nEncodeType);

BOOL HB_NET_setStreamConfig(long lUserID, int nChannel, int streamType, int frameRate, int bitRate, int audio);
    
    typedef struct ST_HB_NET_DEVICEFUNCTION {
        BYTE        functions[256];
    }HB_NET_DEVICEFUNCTION, *LPHB_NET_DEVICEFUNCTION;
    
    BOOL HB_NET_GetDeviceSupportFunc(long lUserID, LPHB_NET_DEVICEFUNCTION pFunctions);
